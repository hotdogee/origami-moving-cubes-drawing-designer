...
class JogWheel {
  ...
  constructor(nodes, options, window = global.window, document = global.document) {
    ...
    const medias = getMediaqueryMedia(document.styleSheets).map(window.matchMedia)
   ...
   */
  onMatchMedia() {
    const { elements, players, settings, window, document } = this.__instance
   ...
   *
   * // Seek to middle of animation sequence and play
   * wheel.seek(0.5).reverse();
   */
  reverse() {
    this.players.forEach((player) => player.reverse())
    return this
  }
